...
class MainViewController: UIViewController {
    ...
    private func upload(fileUrl: URL, progress: ((Progress) -> Void)? = nil) async throws {
        let uploadedVideo = try await AsyncApiUtils.uploadVideoWithUploadToken(uploadToken: SettingsManager.uploadToken, url: fileUrl, progress: progress)

        print("Video uploaded: \(uploadedVideo)")
        AlertUtils.show("File has been successfully uploaded at video ID: \(uploadedVideo.videoId)", title: "Success", vc: self)
    }
}
...
extension MainViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        ApiVideoUploader.basePath = SettingsManager.environment.rawValue

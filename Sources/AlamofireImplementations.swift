...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = ApiVideoUploader.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> URLSessionTask? {
        let managerId = UUID().uuidString
        // Create a new manager for each request to customize its request header
        var manager: Alamofire.Session

        if URLString.hasSuffix("/auth/api-key") {
            manager = createAlamofireSession()
        } else if let apiKey = ApiVideoUploader.apiKey {
            // Create the interceptor
            let authenticator = ApiVideoAuthenticator(apiKey: apiKey)
            let interceptor = AuthenticationInterceptor(authenticator: authenticator, credential: ApiVideoUploader.credential)

            manager = createAlamofireSession(interceptor: interceptor)
        } else {
            manager = createAlamofireSession()
        }
                ...
                let upload = manager.upload(multipartFormData: { mpForm in
                    for (k, v) in self.parameters! {
                        ...
                        case let chunkInputStream as ChunkInputStream:
                            let fileURL = chunkInputStream.fileURL
                            let headers = self.contentHeaders(withName: k, fileName: fileURL.lastPathComponent, mimeType: "video/quicktime")
                            mpForm.append(chunkInputStream, withLength: UInt64(chunkInputStream.maxSize), headers: headers)
            ...
            validatedRequest.responseData(queue: apiResponseQueue, completionHandler: { dataResponse in
                ...
                do {
                    ...
                    if let headerFileName = self.getFileName(fromContentDisposition: dataResponse.response?.allHeaderFields["Content-Disposition"] as? String) {
                        ...
                        requestPath = requestPath.appending("/tmp.ApiVideoUploader.\(UUID().uuidString)")
                    }
        ...
        default:
            validatedRequest.responseData(queue: apiResponseQueue, completionHandler: { dataResponse in
                ...
                if case let .failure(error) = dataResponse.result {
                    ...
                    // Return status code of underlyingError for interception error mainly
                    if let underlyingErrorResponse = error.underlyingError as? ErrorResponse {
                        if case let ErrorResponse.error(code, _, _, _) =  underlyingErrorResponse {
                            statusCode = code
                        }
                    }
                    completion(.failure(ErrorResponse.error(statusCode ?? 500, dataResponse.data, dataResponse.response, error)))
                    return
                }

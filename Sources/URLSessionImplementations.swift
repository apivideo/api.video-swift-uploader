...
public typealias ApiVideoUploaderChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, ApiVideoUploaderChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: ApiVideoUploaderChallengeHandler?

    /**
     To observe upload or download progress
     */
    public var progressObservation: NSKeyValueObservation?
    ...
    open func createURLRequest(urlSession: URLSessionProtocol, method: HTTPMethod, encoding: ParameterEncoding, headers: [String: String]) throws -> URLRequest {
        ...
        originalRequest.timeoutInterval = ApiVideoUploader.timeout
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in ApiVideoUploader.customHeaders {
            httpHeaders[key] = value
        }
        return httpHeaders
    }
...
open class URLSessionDecodableBackgroundUploadRequestBuilder<T: Decodable>: URLSessionDecodableRequestBuilder<T> {
